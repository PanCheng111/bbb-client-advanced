package org.bigbluebutton.view.navigation.pages.documentDetails {
	...
	public class DocumentDetailsViewMediator extends Mediator {
		
		[Inject]
		public var view:IDocumentDetailsView;
		...
		override public function initialize():void {
			...
			setPresentation(userUISession.currentPageDetails as Presentation);
		...
		private function stageOrientationChangingHandler(e:Event):void {
			if (userUISession.currentPage == PagesENUM.DOCUMENT_DETAILS) { //apply rotation only if user didn´t change view at the same time
...
//		}
		...
		private function setPresentation(p:Presentation):void {
			...
			if (_currentPresentation != null) {
				...
				var num:int = p.slides.length;
				for (var i:int = 0; i < num; i++)
					if (!p.getSlideAt(i).loaded) {
						var slide:Slide = p.getSlideAt(i);
						slide.slideLoadedSignal.remove(slideLoadedHandler);
						loadSlideSignal.dispatch(slide, _currentPresentation.id);
					}
